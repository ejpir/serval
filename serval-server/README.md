...
The terminated `h2/server.zig` path receives `std.Io` from downstream h2 entry points and keeps plain h2 transport on `std.Io` stream reader/writer operations so connection fibers yield naturally under backpressure and cancellation. TLS h2 transport now routes through `serval-socket.terminated` bounded read/write driver APIs (which consume `serval-tls` step semantics) so `WANT_READ`/`WANT_WRITE` handling stays centralized outside frame logic. The HTTP/1 TLS path also now uses the same terminated-driver contract for connection reads/writes, while h1 plain-socket transport stays on `std.Io` for consistent fiber behavior. Downstream terminated h2 TLS reads use `serval-core.config.H2_SERVER_IDLE_TIMEOUT_NS` as the bounded readiness budget; when this timeout is hit, the affected connection is closed fail-closed instead of waiting indefinitely. On the HTTP/1 TLS path, `Config.tls.handshake_timeout_ns` bounds the server-side handshake and `Config.tls.io_timeout_ns` bounds subsequent TLS reads and writes, preventing slowloris-style clients from pinning a worker indefinitely before or during request processing.
