...
# Run through the crash-safe wrapper and print a postmortem failure summary
integration/run_with_failure_summary.sh

# Run a focused step through the same wrapper
integration/run_with_failure_summary.sh test-integration-42
```
...
When a run fails, the custom runner now prints a final `Failed tests:` section
with the test index, friendly name, and failure reason so you do not need to
scroll back through the whole buffer to find the failures.
If the test process crashes before the runner can print that summary, use
`integration/run_with_failure_summary.sh`; it saves the full log to `/tmp` and
prints the observed failing tests and/or the last incomplete test after the
process exits.
...
Focused rerun/debug commands for the NetBird runtime DSL split regression:

- `zig build test-integration-37`
- `zig build test-integration-37-gdb`

Focused rerun/debug commands for the upstream-session-pool reuse regression:

- `zig build test-integration-42`
- `zig build test-integration-42-gdb`

Focused rerun command for the grpcurl metadata/trailers regression:

- `zig build test-integration-70`

Focused rerun command for the h2c GOAWAY last-stream-id regression:

- `zig build test-integration-92`

Focused rerun command for the h2c GOAWAY session-rollover regression:

- `zig build test-integration-93`

Focused rerun command for the h2c mixed GOAWAY/non-gRPC soak regression:

- `zig build test-integration-99`

Focused rerun command for the missing grpc-status trailer regression:

- `zig build test-integration-101`

Focused rerun/debug commands for the h2c near-concurrent-stream-bound churn regression:

- `zig build test-integration-105`
- `zig build test-integration-105-gdb`
- `zig build test-integration-105-loop`

The loop step is bounded. Override the iteration count with `SERVAL_LOOP_COUNT`
when you need a longer chase:

```bash
SERVAL_LOOP_COUNT=250 zig build test-integration-105-loop
```
