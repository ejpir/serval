...
pub const Watcher = struct {
    ...
    pub fn init(
        ...
        assert(controller_name.len <= types.MAX_CONTROLLER_NAME_LEN); // S1: precondition - controller name fits
    ...
    fn parseStoredGatewayClasses(self: *Self) WatcherError!void {
        ...
        while (iteration < MAX_GATEWAY_CLASSES) : (iteration += 1) {
            ...
            try parsing.parseGatewayClassJson(raw_json, &self.parsed_gateway_classes[idx]);
    ...
    pub fn gatewayClassMatches(gateway_class_name: []const u8, our_classes: []const []const u8) bool {
    ...
    fn parseStoredHTTPRoutes(self: *Self) WatcherError!void {
        ...
        while (iteration < MAX_HTTP_ROUTES) : (iteration += 1) {
            ...
            try parsing.parseHTTPRouteJson(raw_json, &self.parsed_http_routes[idx]);
