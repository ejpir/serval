...
/// Errors returned by `generateMaterials`.
/// `InvalidDomain` means the input domain was empty or exceeded `config.ACME_MAX_DOMAIN_NAME_LEN`.
/// `OutputTooSmall` means one of the caller-provided PEM buffers could not hold the encoded result.
...
/// PEM output generated by `generateMaterials`.
/// `cert_pem` and `key_pem` borrow storage owned by the caller-provided output buffers.
/// Keep the backing buffers alive and unchanged for as long as these slices are used.
...
/// Generates an ACME certificate chain and matching EC private key for `domain`.
/// Writes PEM-encoded certificate and key material into the provided output buffers and returns borrowed slices into them.
/// Fails with `error.InvalidDomain` if `domain` is empty or longer than `config.ACME_MAX_DOMAIN_NAME_LEN`, `error.OutputTooSmall` if either buffer cannot hold the encoded output, and `error.SignatureFailed` if signing the certificate fails.
/// The returned slices remain valid until the caller reuses or overwrites `cert_pem_buf` or `key_pem_buf`.
