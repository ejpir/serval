...
pub fn main() !void {
    ...
    log.info("=== BoringSSL TLS Server POC ===", .{});
    ...
    if (ssl.SSL_CTX_use_certificate_chain_file(ctx, cert_path_z) != 1) {
        log.err("Failed to load certificate from {s}", .{cert_path});
        ssl.printErrors();
        return error.LoadCertFailed;
    }
    log.info("Loaded certificate from {s}", .{cert_path});

    if (ssl.SSL_CTX_use_PrivateKey_file(ctx, key_path_z, ssl.SSL_FILETYPE_PEM) != 1) {
        log.err("Failed to load private key from {s}", .{key_path});
        ssl.printErrors();
    ...
    while (true) {
        ...
        if (ret != 1) {
            ...
            log.err("SSL_accept failed: {d}", .{err});
            ssl.printErrors();
            continue;
        }
        ...
        if (n <= 0) {
            ...
            log.err("SSL_read failed: {d}", .{err});
            continue;
        }

        const bytes_read: usize = @intCast(n);
        log.info("Received {d} bytes:", .{bytes_read});
        std.debug.print("{s}\n", .{buf[0..bytes_read]});
        ...
        log.info("Sent {d} bytes response", .{written});
